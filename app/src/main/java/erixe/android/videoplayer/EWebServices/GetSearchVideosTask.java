...
public class GetSearchVideosTask extends AsyncTask<String, Void, WebServiceRespond> {

    private OnTaskCompleteListener listener;
    private static final String URL = EWebServiceStrings.GET_SEARCH_TASK_URL;
    private static final String METHOD_TYPE = EWebServiceStrings.METHOD_TYPE_GET;
    private Map<String, String> paramsMap;

    public GetSearchVideosTask(OnTaskCompleteListener listener, Map<String, String> paramsMap)
    {
    ...
    protected void onPostExecute(WebServiceRespond preWebServiceRespond) {
        if(!preWebServiceRespond.ok)
            listener.onGetSearchVideosTaskComplete(preWebServiceRespond, null);
        else {
            WebServiceRespond initServerRespond = Utilities.initializeWebServiceRespond(preWebServiceRespond);
            if(initServerRespond.ok)
                listener.onGetSearchVideosTaskComplete(initServerRespond, analyzeJsonString(initServerRespond.result));
            else
                listener.onGetSearchVideosTaskComplete(initServerRespond, null);
        }
    }
